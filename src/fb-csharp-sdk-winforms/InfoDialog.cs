...
namespace fb_csharp_sdk_winforms
{
    ...
    public partial class InfoDialog : Form
    {
        ...
        public List<dynamic> pages = new List<dynamic>();
        public List<dynamic> pageids = new List<dynamic>();
        public List<dynamic> pagetokens = new List<dynamic>();
        public FacebookClient pageclient;
        public dynamic currentaccesstoken;
        public dynamic currentpage;

        public List<string> images;
        public double timeleft;
        public TimeSpan totaltimespan;
        public int siradakino = 0;
        ...
        private void GraphApiExample()
        {
            ...
            try
            {
                ...
                dynamic rrr = fb.Get("me/accounts?fields=access_token,name");
                dynamic data = rrr.data;

                foreach (dynamic key in data) {
                    pagetokens.Add(key.access_token);
                    pages.Add(key.name);
                    pageids.Add(key.id);
        ...
        private void renewAccessTokens() {
            try
            {
                var fb = new FacebookClient(_accessToken);
        ...
        private void listBox1_MouseDoubleClick(dynamic sender, MouseEventArgs e)
        {
            if(!panel1.Visible)
                panel1.Show();
            label2.Text = sender.Text;
            currentaccesstoken = pagetokens[sender.SelectedIndex];
            currentpage = pageids[sender.SelectedIndex];
        }

        private void button1_Click(object sender, EventArgs args)
        {
            pageclient = new FacebookClient(currentaccesstoken);

            pageclient.PostCompleted += (o, e) =>
            {
                if (e.Cancelled)
                {
                    var cancellationError = e.Error;
            ...
            dynamic parameters = new ExpandoObject();
            parameters.message = richTextBox1.Text;
            pageclient.PostAsync(currentpage + "/feed", parameters);
        }
            ...
            try
            {
                ...
                using (FolderBrowserDialog dialog = new FolderBrowserDialog())
                {
                    ...
                    if (dialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (images.Count > 0)
                        {
                            label3.Text = images.Count.ToString() + " " + folder;
                            button4.Enabled = true;
                        }
                        else { 
                            label3.Text = "Folder does not contain images";
        ...
        private void setTimer_Click(object sender, EventArgs e)
        {
            ...
            //totaltimespan = new TimeSpan(1, new Random().Next(-10, 10), 0);
            totaltimespan = new TimeSpan(0, 0, 5);
            timer1.Interval = 1000;
            timer1.Enabled = true;
            timeleft = new TimeSpan(1, new Random().Next(-10, 10), 0).TotalSeconds;
        }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            {
                if (siradakino < images.Count)
                {
                    System.Diagnostics.Debug.WriteLine(images[siradakino]);
                    post_image(images[siradakino]);
                    siradakino += 1;
                    timeleft = new TimeSpan(1, new Random().Next(-10, 10), 0).TotalSeconds;
                }
                else
                {
                    label4.Text = "All images Shown";
                    timer1.Stop();
                }
            }
        }

        private void post_image(string path)
        {
            renewAccessTokens();
            currentaccesstoken = pagetokens[pageids.IndexOf(currentpage)];
            pageclient = new FacebookClient(currentaccesstoken);
            pageclient.PostCompleted += (o, e) =>
            {
                if (e.Cancelled)
                ...
                else if (e.Error != null)
                {
                    ...
                    this.BeginInvoke(new MethodInvoker(
                                                 () =>
                                                 {
                                                     listBox2.Items.Add(path + " ! " + e.Error.ToString());
                                                 }));
                }
                else
                {
                    ...
                    this.BeginInvoke(new MethodInvoker(
                                         () =>
                                         {
                                             ...
                                             if (!Directory.Exists(path.Replace(Path.GetFileName(path), "done\\")))
                                                 Directory.CreateDirectory(path.Replace(Path.GetFileName(path), "done\\"));
                                             if (File.Exists(path))
                                                 File.Move(path, path.Replace(Path.GetFileName(path), "done\\" + Path.GetFileName(path)));
                                         }));
                }
            };
            ...
            pageclient.PostAsync(currentpage + "/photos", parameters);
        }
